...
def remote_check_stdio(code, stdin, stdout):
    ...
    try:
        ...
        print(f"Error end time: {time.time() - start}")
        return False, str(e), stdin, stdout
...
CODE_PATTERN = re.compile(r"```python(.*?)```", re.DOTALL)  # 使用 re.DOTALL 标志 [[1]]
...
def _compute_score(solution_str, ground_truth, extra_info, format_reward=0., answer_reward=1.):
    reward_log = []
    ...
    if len(solution_code) == 0:  # only print full output when there is an error
    ...
    if "pytest" in ground_truth or "functional" in ground_truth:
        try:
            ...
            print(f"Error end time1: {time.time() - start}")
            succ = False
            output = str(e)
        if not succ:
...
def compute_score(solution_str, ground_truth, extra_info, format_reward=-1.0, answer_reward=1.0):
    ...
    marker = "✅" if score == answer_reward else "❌"
